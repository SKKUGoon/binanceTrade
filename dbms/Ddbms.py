...
class LocalDBMethods2:
    ...
    LocalDBMethods contains methods for
        ...
        2) Creating table
        3) Inserting serverconn
        4) Replacing serverconn
        5) Updating serverconn
    ...
    def _execute_query(self, query: str, multiple: bool, new_vals=None):
        ...
        :param new_vals: If new_vals is not None, user would be replacing,inserting,updating the serverconn
        """
        c = self.conn.cursor()
        if new_vals is None:
            c.execute(query)
...
class MySQLDBMethod:
    ...
    def __init__(self, configure, db, test=False):
        ...
        if db == 'main':
            ...
            id, pw, db_ip, db_name = 'root', '0000', '127.0.0.1', 'main'
    ...
    def commit(self, enforce=False):
        ...
        if enforce is False:
            # Check commit
            print("Are you sure you want to change your serverconn? y/n")
            ans = input()
            if ans == 'y':
